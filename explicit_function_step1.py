"""
LATEST VERSION 28 JANUARY 2023
...
def explicit_function(PDB_input_file,final_pdb_file, DCD_output_file,temp):
    ...
    simulation.step(1000000)  #equilibrate for 1 ns
    
    # Adding Reporters -- need to change this
    print("Collecting Data...")
    simulation.reporters.append(DCDReporter(DCD_output_file, 1000)) #Every 100ps record one traj data
    simulation.reporters.append(StateDataReporter(stdout, 1000, step=True, potentialEnergy=True, temperature=True, separator=' ')) 
    simulation.reporters.append(PDBReporter(final_pdb_file, 1000, enforcePeriodicBox=True)) 
